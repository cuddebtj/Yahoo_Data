...
def yahoo_update():
    ...
    nfl_dates_df = pd.read_csv(
        os.path.join(path, "nfl-weeks.csv"), parse_dates=["Start_Date", "End_Date"]
    )
    ...
    spread.df_to_sheet(
        weeks_df, index=False, headers=False, start=(2, 2), replace=False, fill_value=""
    )

    with open(path + "/cronoutput.txt", "a") as f:
        f.write(
            "Done: Regular season update on "
